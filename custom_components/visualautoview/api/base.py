...
from homeassistant.const import HTTP_OK, HTTP_CREATED, HTTP_BAD_REQUEST, HTTP_NOT_FOUND, HTTP_INTERNAL_SERVER_ERROR
...
class WebSocketHandler(ABC):
    ...
    async def send_response(self, connection, message_id: str, data: Any, success: bool = True):
    ...
    async def broadcast_event(self, event_type: str, data: Any, subscription_filter: Optional[Callable] = None):
        ...
        for sub_id, subscription in self._subscriptions.items():
            if subscription_filter is None or subscription_filter(subscription):
                try:
                    ...
                    self._logger.error(f"Failed to send event to subscription {sub_id}: {e}")
...
class ApiErrorHandler:
    ...
    @staticmethod
    def handle_error(error: Exception, status: int = HTTP_INTERNAL_SERVER_ERROR) -> tuple:
