...
class ParseGraphEndpoint(RestApiEndpoint):
        ...
        try:
            ...
            result = {
                ...
                "statistics": {
                    ...
                    "trigger_count": len([n for n in graph.nodes if n.type == "trigger"]),
                    "condition_count": len([n for n in graph.nodes if n.type == "condition"]),
    ...
    async def get(self, request) -> tuple:
        ...
        return self.error_response(
            "GET not supported. Use POST with automation data.",
            HTTP_BAD_REQUEST
        )
...
class GetAutomationGraphEndpoint(RestApiEndpoint):
    ...
    async def get(self, request) -> tuple:
        ...
        try:
            ...
            if f"automation.{automation_id}" not in automations and automation_id not in automations:
                return self.error_response(
            ...
            automation_state = self.hass.states.get(f"automation.{automation_id}") or \
                              self.hass.states.get(automation_id)
    ...
    async def post(self, request) -> tuple:
        ...
        try:
            ...
            if not automation_data:
                return self.error_response(
                    "Missing required field: automation_data",
                    HTTP_BAD_REQUEST
                )
            ...
            if valid:
                try:
                    parser = AutomationGraphParser()
                    graph = parser.parse_automation(automation_data)
                    statistics["triggers"] = len([n for n in graph.nodes if n.type == "trigger"])
                    statistics["conditions"] = len([n for n in graph.nodes if n.type == "condition"])
                    statistics["actions"] = len([n for n in graph.nodes if n.type == "action"])
                except Exception as e:
                    if strict:
                        valid = False
