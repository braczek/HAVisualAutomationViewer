...
class DependencyGraphService:
    ...
    async def build_dependency_graph(self) -> DependencyGraph:
        ...
        try:
                        ...
                        if isinstance(trigger, dict):
                            ...
                            if trigger_entity:
                                if isinstance(trigger_entity, list):
                                    if any(e in source_entities for e in trigger_entity):
            ...
            if graph.total_automations > 0:
                graph.avg_chain_length = graph.total_dependencies / max(1, graph.total_automations)
            ...
            async def trace_chain(start_id: str, current_chain: list[str]) -> None:
                ...
                for edge in graph.edges:
                    if edge.source_automation_id == start_id:
                        ...
                        await trace_chain(edge.target_automation_id, current_chain.copy())
                
                if not found_dependency and len(current_chain) > 1:
                    # End of chain
                    automations_config = self.hass.data.get("automation", {})
                    aliases = [automations_config.get(aid, {}).get("alias", aid) for aid in current_chain]
                    
                    chain = DependencyChain(
                        automations=current_chain.copy(),
                        aliases=aliases,
                        total_estimated_duration=len(current_chain) * 100,  # 100ms per automation
                        is_circular=False,
                        risk_level="low",
                    )
                    chains.append(chain)
                ...
                for edge in graph.edges:
                    if edge.source_automation_id == node:
                        ...
                        if neighbor not in visited:
                            ...
                            automations_config = self.hass.data.get("automation", {})
                            aliases = [automations_config.get(aid, {}).get("alias", aid) for aid in cycle[:-1]]
                            ...
                            chain = DependencyChain(
                                ...
                                is_circular=True,
                                risk_level="high",
                                potential_issues=["Circular dependency detected", "Could cause infinite execution loops"],
                            )
    ...
    async def analyze_automation_impact(
        ...
        try:
            ...
            def find_cascade(auto_id: str, depth: int = 0, max_depth: int = 10) -> None:
                ...
                for edge in graph.edges:
                    if edge.source_automation_id == auto_id and edge.target_automation_id not in affected_automations:
                        affected_automations.add(edge.target_automation_id)
                        cascade_count += 1
            ...
            return {
                ...
                "risk_level": "high" if len(affected_automations) > 5 else "medium" if len(affected_automations) > 2 else "low",
            }
    ...
    def calculate_chain_risk(self, chain: DependencyChain) -> dict[str, Any]:
        ...
        if chain.total_estimated_duration > 5000:  # 5 seconds
            risk_score += 0.2
            issues.append(f"Long execution duration: {chain.total_estimated_duration}ms")
        ...
        try:
            ...
            def add_to_execution(auto_id: str, depth: int = 0) -> None:
                ...
                execution_order.append({
                    ...
                    "expected_start_ms": sum(100 for _ in range(len(execution_order))),
                })
