...
class ComparisonEngine:
        ...
        try:
            ...
            for i, auto1_id in enumerate(automation_ids):
                for auto2_id in automation_ids[i+1:]:
                    diff = self._generate_diff(automations_data[auto1_id], automations_data[auto2_id])
                    diff.automation_id_1 = auto1_id
                    diff.automation_id_2 = auto2_id
                    comparisons.append(diff.to_dict())
            
            # Generate consolidation suggestions
            diffs = [self._generate_diff(automations_data[auto_ids[0]], automations_data[auto_ids[1]]) 
                    for auto_ids in [(automation_ids[i], automation_ids[i+1]) 
            ...
            _LOGGER.info(f"Comparison complete: {len(comparisons)} comparisons, {len(suggestions)} suggestions")
            return result
    ...
    async def find_similar(
        ...
        """
        try:
            _LOGGER.info(f"Finding similar automations to {automation_id} (threshold: {threshold})")
            ...
            result = sorted(filtered, key=lambda x: x['similarity_score'], reverse=True)[:limit]
    ...
    def _calculate_similarity(self, auto1: Dict[str, Any], auto2: Dict[str, Any]) -> float:
        """
    ...
    def _generate_diff(self, auto1: Dict[str, Any], auto2: Dict[str, Any]) -> AutomationDiff:
        """
        ...
        try:
            ...
            conditions_diff = self._diff_component_lists(
                auto1.get('conditions', []),
                auto2.get('conditions', []),
                'condition'
            )
            ...
            # Count differences
            total_differences = len(triggers_diff) + len(conditions_diff) + len(actions_diff) + len(metadata_diff)
            ...
            for diff in diffs:
                ...
                if diff.similarity_score > 80:
                    suggestion = ConsolidationSuggestion(
                        ...
                        estimated_components_reduction=int(diff.total_differences * 0.7),
                        potential_benefits=[
                ...
                # Medium similarity (60-80%) - moderate consolidation
                    ...
                    suggestion = ConsolidationSuggestion(
                        ...
                        estimated_components_reduction=int(diff.total_differences * 0.4),
                        potential_benefits=[
                            "Reduced redundancy",
