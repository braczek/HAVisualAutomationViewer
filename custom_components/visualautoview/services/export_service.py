...
class ExportService:
    ...
    async def _export_png(self, automation_id: str, options: ExportOptions) -> ExportResult:
        """Export as PNG image."""
        import time
        import os
        
        start_time = time.time()
        _LOGGER.debug(f"Exporting {automation_id} as PNG with quality {options.quality}")
        
        try:
            # Get quality preset
            preset = self.QUALITY_PRESETS.get(options.quality, self.QUALITY_PRESETS['medium'])
            ...
            file_size = preset['max_width'] * preset['max_height'] * 4  # Approximate size
    ...
    async def _export_svg(self, automation_id: str, options: ExportOptions) -> ExportResult:
        """Export as SVG image."""
        import time
        import os
...
<svg xmlns="http://www.w3.org/2000/svg" width="{options.width}" height="{options.height}">
  ...
  <g id="automation_{automation_id}">
    ...
    async def _export_pdf(self, automation_id: str, options: ExportOptions) -> ExportResult:
