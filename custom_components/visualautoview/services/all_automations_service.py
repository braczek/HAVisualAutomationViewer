...
class DashboardFilter:
    ...
    def applies(self, automation: MiniGraphData) -> bool:
        ...
        if self.trigger_platforms:
            if not any(tp in automation.primary_triggers for tp in self.trigger_platforms):
                return False
...
class AllAutomationsService:
    ...
    async def get_automation_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_nodes = sum(auto.node_count for auto in automations) / len(automations) if automations else 0
            avg_edges = sum(auto.edge_count for auto in automations) / len(automations) if automations else 0
